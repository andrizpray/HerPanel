...
export default function Edit({ ftpUser, domains }) {
    const { data, setData, put, processing, errors } = useForm({
        domain_id: ftpUser.domain_id || '',
        username: ftpUser.username,
        password: '', // leave blank for no change
        quota_mb: ftpUser.quota_mb,
        directory: ftpUser.directory || '',
        status: ftpUser.status,
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('ftp.update', ftpUser.id));
    }

    return (
        <AuthenticatedLayout
            header={<h2 className="font-semibold text-xl text-gray-200 leading-tight">Edit FTP User</h2>}
        >
            <div className="py-12">

...
export default function Edit({ domainId, redirectRule }) {
  const { data, setData, put, processing, errors } = useForm({
    source_path: redirectRule.source_path || '',
    destination_url: redirectRule.destination_url || '',
    redirect_type: redirectRule.redirect_type || '301',
    is_active: redirectRule.is_active ?? true,
    priority: redirectRule.priority || 0,
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    put(`/domains/${domainId}/redirects/${redirectRule.id}`);
  };

  const handleDelete = () => {
    if (confirm('Yakin hapus redirect rule ini?')) {
      router.delete(`/domains/${domainId}/redirects/${redirectRule.id}`);
    }
  };
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-100">Edit Redirect Rule</h1>
        <a

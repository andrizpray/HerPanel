<?php
// Test script for HerPanel features - CORRECTED
...
// 2. EMAIL (EmailAccount with virtual_users table)
echo "\n2. EMAIL MANAGEMENT (EmailAccount)\n";
...
echo "Email Account ID: {$email->id}\n";
...
// Test route accessibility (will redirect to login)
echo "\n=== TESTING ROUTE ACCESS (expect 302 redirect) ===\n";
$routes = ['domains', 'emails', 'databases', 'firewall', 'backups'];
foreach ($routes as $route) {
    $output = [];
    $return_var = 0;
    exec("curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:8000/{$route}", $output, $return_var);
    $code = $output[0] ?? '???';
